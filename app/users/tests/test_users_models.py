...
class UserModelTests(TestCase):
    ...
    def test_user_str_with_names(self):
        ...
        user = User.objects.create_user(**self.user_data)
        expected_str = "test@example.com (patient)"
    ...
    def test_user_str_without_names(self):
        ...
        self.assertEqual(str(user), "test@example.com (patient)")
...
class UserModelSecurityTests(TestCase):
    ...
    #     self.assertIn("onerror", user.name)
...
class UserModelPerformanceTests(TestCase):
    ...
    def test_bulk_user_creation(self):
        ...
        users_data = [
            User(email=f"user{i}@example.com", is_active=True) for i in range(100)
        ]
