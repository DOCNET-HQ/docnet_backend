...
from utils.permissions import IsHospital
...
class DoctorKYCRecordListView(generics.ListAPIView):
    ...
    search_fields = [
        'doctor__name', 'doctor__registration_number', 'reason'
    ]
...
class DoctorKYCRecordsForDoctorView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Admins can view any
        if (
            doctor.user != self.request.user and
            not self.request.user.is_staff
        ):
            return DoctorKYCRecord.objects.none()

        return DoctorKYCRecord.objects.filter(
            doctor=doctor
        ).select_related(
            'doctor', 'reviewed_by'
...
def Doctor_stats(request):
    ...
    stats = {
        ...
        'kyc_completion_rate': round(
            (approved_kyc / total_doctors * 100)
            if total_doctors > 0 else 0, 2
        )
    }
