...
class HospitalFilter(django_filters.FilterSet):
    ...
    # Status filters
    kyc_status = django_filters.ChoiceFilter(
        choices=[
            ("PENDING", "Pending"),
            ("APPROVED", "Approved"),
            ("REJECTED", "Rejected"),
    ...
    license_authority = django_filters.CharFilter(
        field_name="license_issuance_authority", lookup_expr="icontains"
    )

    # Fixed Specialties filter - supports single or multiple specialty names
    specialties = django_filters.CharFilter(
        method="filter_specialties", label="Specialties"
    )

    # Search across multiple fields
    search = django_filters.CharFilter(method="filter_search")

    def filter_specialties(self, queryset, name, value):
        """
        Filter hospitals by specialty names
        Supports:
        - Single specialty: ?specialties=Cardiology
        - Multiple specialties (AND logic): ?specialties=Cardiology,Neurology
        """
        if value:
            # Split comma-separated values
            specialties = [spec.strip() for spec in value.split(",")]

            # Apply AND logic - hospital must have ALL specified specialties
...
class HospitalKYCRecordFilter(django_filters.FilterSet):
    ...
    hospital_name = django_filters.CharFilter(
        field_name="hospital__name", lookup_expr="icontains"
    )
    hospital_registration = django_filters.CharFilter(
        field_name="hospital__registration_number", lookup_expr="icontains"
    )
    ...
    reviewed_after = django_filters.DateTimeFilter(
        field_name="reviewed_at", lookup_expr="gte"
    )
    reviewed_before = django_filters.DateTimeFilter(
        field_name="reviewed_at", lookup_expr="lte"
    )
    reviewed_on = django_filters.DateFilter(
        field_name="reviewed_at", lookup_expr="date"
    )

    # Reviewer filters
    reviewed_by_username = django_filters.CharFilter(
        field_name="reviewed_by__username", lookup_expr="icontains"
    )
